...
export async function createTaskAction(taskData: Omit<Task, "id" | "status"> & { id?: number }): Promise<void> {
  const { data, content } = await readTasksFile();
  const tasks = data.items || [];
  
  const deadline = new Date(taskData.eta);
  const now = new Date();
  const status: Task['status'] = deadline > now ? "In Progress" : "Overdue";
